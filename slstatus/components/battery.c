 ...
 */
	...
	int
	battery_perc_helper(const char *bat)
	{
		int cap_perc;
		char path[PATH_MAX];
	...
	battery_state(const char *bat)
	{
    	...
    	int
	battery_state_helper(const char *bat)
	{
		char path[PATH_MAX], state[12];
		...
		if (pscanf(path, "%12[a-zA-Z ]", state) != 1)
			return 0;
                ...
                return !strcmp("Charging", state);
	}

        const char *
        battery(const char *bat) {
            int cap_perc = battery_perc_helper(bat);
            int is_charging = battery_state_helper(bat);
            if (cap_perc <= 15 && !is_charging) {
                    return bprintf("^c#FF0000^[BAT %d%%]^c#FFFFFF^", cap_perc);
            } else {
                    if (is_charging) {
